...
# Original Source Code: Made Available by skimage developers
# Modified by: Aman Bhatta
# Work Based on this paper: https://hal.science/hal-00232709
...
with header:
    st.markdown("<h1 style='text-align: center; color: green;'>Perceptual Blur Estimator!", unsafe_allow_html=True)
    st.markdown("<h6 style='text-align: center; color: green;'>Based on: https://hal.science/hal-00232709</h6>", unsafe_allow_html=True)
    image = st.file_uploader("Upload Image here!",label_visibility="hidden")
    
   #st.write(np.load(image1).shape)
    if image is not None:
        sk_io_image = skimage.io.imread(image)
        
        # pil image for display
        pil_image = Image.fromarray(np.array(sk_io_image))
        pil_image = pil_image.resize((224, 224))
        st.image(pil_image)

        
        blur_estimation = estimate_blur_perceptual(sk_io_image)
        ...
        <p class="a"> Estimated Blur Score = {blur_estimation:.3f} <br> Score Range - [0-1] <br> Lower is better/less blur</p>
        """

        st.markdown(html_str, unsafe_allow_html=True)
    else:
        st.markdown("<h3 style='text-align: center; color: red;'>Upload Image to get the Blur Estimate</h3>", unsafe_allow_html=True)
